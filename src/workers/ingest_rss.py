"""RSS/Atom ingestion worker: cron dispatcher + per-source job."""
...
async def poll_rss_sources(ctx: dict) -> None:
    """Cron dispatcher: queries active RSS/Atom sources and enqueues per-source jobs."""
    ...
    for source in sources:
        await ctx["redis"].enqueue_job(
            "ingest_rss_source", source.id, _queue_name="fast"
        )
    ...
    logger.info("poll_rss_sources_dispatched", count=len(sources))


async def ingest_rss_source(ctx: dict, source_id: str) -> None:
    """Per-source job: fetch, parse, deduplicate, and store RSS/Atom entries."""
    ...
    async with _db.async_session_factory() as session:
        ...
        try:
            ...
            if content is None:
                ...
                logger.info("ingest_rss_not_modified", source_id=source_id)
                return
            ...
            for entry in parsed.entries:
                ...
                summary_text = re.sub(
                    r"<[^>]+>", "", str(entry.get("summary") or "")
                ).strip()

                # Fetch full article body when RSS content is thin
                if len(summary_text) < 50 and entry_url:
                    fetched_body = await fetch_article_body(entry_url)
                    if fetched_body:
                        summary_text = fetched_body
            ...
            await handle_source_success(session, source, new_etag, new_lm)
            logger.info("ingest_rss_complete", source_id=source_id, new_items=new_count)
